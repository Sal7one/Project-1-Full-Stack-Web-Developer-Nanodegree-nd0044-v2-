    ...
    for place_deatiles in citites:
        ...
        for venue in venues:
            ...
            if (venue.city == place_deatiles[0]) and (venue.state == place_deatiles[1]):
                ...
                all_venue_shows = db.session.query(Show).filter_by(venue_id = venue.id).all()
...
def delete_venue(venue_id):
  ...
  # Doesn't work if related to a show IM NOT SURE IF IM ALLOWED TO CHANGE THE SCHEMA
  try:
...
def shows():
  ...
  try:
    for a_show in shows:
      ...
      venue = db.session.query(Venue).get(a_show.venue)
      show_Object = {
        "venue_id": venue.id,
        "venue_name": venue.name,
        "artist_id": artist.id,
